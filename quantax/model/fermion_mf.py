...
def _get_pfaffian_indices(sublattice, N):
    if sublattice is None:
        ...
        full_index = jnp.asarray(full_index.reshape(N,N)).astype(jnp.uint32)
        sub_inds = jnp.asarray(sub_inds).astype(jnp.uint32)
        index = jnp.argsort(jnp.asarray(index).astype(jnp.uint32).ravel() - 1)
